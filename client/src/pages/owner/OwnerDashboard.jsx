        ...
        const fetchData = async () => {
            try {
                const [moviesRes, theatersRes, statsRes] = await Promise.all([
                    ...
                    axios.get(import.meta.env.VITE_API_URL + "/api/owner/theater", { headers: { Authorization: `Bearer ${token}` } }),
                    axios.get(import.meta.env.VITE_API_URL + "/api/owner/dashboard-stats", { headers: { Authorization: `Bearer ${token}` } })
                ]);
            ...
            {showTheaterModal && (
                <Modal title="Add Theater" onClose={() => setShowTheaterModal(false)}>
                    <form className="space-y-4" onSubmit={async (e) => {
                        e.preventDefault();
                        await axios.post(import.meta.env.VITE_API_URL + "/api/owner/theater", theaterForm, { headers: { Authorization: `Bearer ${token}` } });
...
const CreateShowForm = ({ theaters, movies, token, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        await axios.post(import.meta.env.VITE_API_URL + "/api/owner/show", { ...form, basePrice: Number(form.basePrice) }, { headers: { Authorization: `Bearer ${token}` } });
        onSuccess();
    };
